...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i = 0; i < sectors_count; i++) {
        ...
        if(sector_read(from_sector + i, buffer) != 0){
            continue;
            // return FIND_FULL;
        }
        ...
        for(size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE) {
            ...
            if(is_lfn(dir->DIR_Attr) || name[0] == NAME_DELETED){
                continue;
            }
            else if(check_name(name,len,dir) && is_valid(dir)){
                slot->dir = *dir;
                slot->offset = off;
                slot->sector = from_sector + i;
                printf("------------------afterTODO1.3:%d\n",FIND_EXIST);
                return FIND_EXIST;
            }
    ...
    *remains += strspn(*remains, "/");    // 跳过开头的'/'
    ...
    while (**remains != '\0' && state == FIND_EXIST) {
        ...
        if(*next_level != '\0') {
            // 不是最后一级，且没找到
            if(state != FIND_EXIST) {
                printf("----------------------------1.1 1.2 warning1\n");
                return -ENOENT;
            }

            // 不是最后一级，且不是目录
            if(!is_directory(slot->dir.DIR_Attr)) {
                printf("----------------------------1.1 1.2 warning2\n");
                return -ENOTDIR;
            }
        }
    }
    printf("----------------------------------afterTODO1.1 1.2: %s\n",path);
    return state;
}
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    cluster_t clus = dir->DIR_FstClusLO + offset / meta.cluster_size;
    size_t p = 0;
    ...
    while(flag != size){
        ...
        int ret = read_from_cluster_at_offset(clus, off, buffer+flag, index_size);
        if(ret < 0){
            return ret;
        }
        offset = 0;
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    while(allocated < n && !is_cluster_end(index)){
...
int file_reserve_clusters(DIR_ENTRY* dir, size_t size) {
    ...
    size_t num_total = (size - 1)/meta.cluster_size + 1;
    if(dir->DIR_FstClusLO == CLUSTER_FREE){
        int ret = alloc_clusters(num_total, &(dir->DIR_FstClusLO));
        if(ret < 0){
            return ret;
        }
        return 0;
    }
    size_t num_now =  (dir->DIR_FileSize - 1)/meta.cluster_size + 1;
    if(num_total <= num_now){
        return 0;
    }
    else{
        cluster_t clus = dir->DIR_FstClusLO;
        while(read_fat_entry(clus) != CLUSTER_END){
            clus = read_fat_entry(clus);
        }
        cluster_t new_clus;
        int ret = alloc_clusters(num_total - num_now, &new_clus);
        if(ret < 0){
            return ret;
        }
        write_fat_entry(clus, new_clus); 
    }
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    file_reserve_clusters(dir, offset + size);
    ...
    off_t off = offset % meta.cluster_size;
    while(p < size){
        size_t index_size = min(meta.cluster_size - off, size - p);
        int ret = write_to_cluster_at_offset(clus, off, data + p, index_size);
        if(ret < 0){
            return ret;
        }
        off = 0;
    ...
    dir->DIR_FileSize = max(size + offset,dir->DIR_FileSize);
...
int fat16_truncate(const char *path, off_t size, struct fuse_file_info* fi) {
    ...
    if(num_total == num_now){
    ...
    else if(num_total > num_now){
        int ret = file_reserve_clusters(dir, size);
        if (ret < 0){
            return ret;
        }
        dir->DIR_FileSize = size;
        ...
        cluster_t new_cluster = read_fat_entry(clus);
        int ret = free_clusters(new_cluster);
        if (ret < 0){
            return ret;
        }
