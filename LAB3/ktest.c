...
// 定义要输出的文件
#define OUTPUT_FILE "/home/auv/桌面/LAB_OS/lab3/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffaf9092a0;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffaf8ea8a0;
        ...
        while(addr < index->vm_end - PAGE_SIZE){
            ...
            if(!IS_ERR_OR_NULL(page)){
                ...
                if(addr <= end && (addr + PAGE_SIZE) >= start){
                    unsigned long from = (addr > start)?v_addr:v_addr + (start - addr);     //用于对齐页框到地址
                    unsigned long offset = ((addr+PAGE_SIZE > end)?end:(addr+PAGE_SIZE)) - ((addr > start)?addr:start);
